...
│ ├── 01\_data\_exploration.ipynb # schema inspection, quality assessment

│ ├── 02\_cleaning.ipynb # cleaning pipeline, BER join

│ ├── 03\_eda.ipynb # trends, geography, energy efficiency

│ └── 04\_modelling.ipynb # modelling and the accuracy ceiling
...
Requires a free FRED-style registration for the SEAI research tool and manual download of both source files.
...
Place `ppr\_raw.csv` and `BERPublicsearch.txt` in `data/raw/`, then run the notebooks in order. The CSO index is fetched automatically via the PxStat API.
...
Python, pandas, scikit-learn, LightGBM, matplotlib, seaborn, requests, pyarrow
